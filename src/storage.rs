...
pub fn save_brain_state(brain: &WormBrain, path: &Path) -> Result<()> {
    ...
    .map_err(|e| {
        CoreError::Bridge(format!(
            "failed to create input_projection TensorView: {}",
            e
        ))
    })?;
    ...
    std::fs::write(path, &serialized).map_err(|e| {
        CoreError::Bridge(format!(
            "failed to write safetensors to {}: {}",
            path.display(),
            e
        ))
    })?;
...
pub fn load_brain_state(path: &Path) -> Result<(Array2<f32>, Array2<f32>)> {
    ...
    let proj = match st.tensor("input_projection") {
        ...
        Err(_) => crate::worm_brain::WormBrain::new_baseline()
            .input_projection()
            .clone(),
    };
...
#[inline]
pub fn save_brain_state_extended(brain: &WormBrain, path: &Path) -> Result<()> {
    ...
    if let Some(ref reservoir) = brain.echo_reservoir {
        ...
        .map_err(|e| {
            CoreError::Bridge(format!("failed to create associations TensorView: {}", e))
        })?;
        tensors.insert("associations", assoc_tensor);

        pre_decay_data = reservoir.pre_decay.to_le_bytes().to_vec();
        let tv =
            safetensors::tensor::TensorView::new(safetensors::Dtype::F32, vec![1], &pre_decay_data)
                .map_err(|e| {
                    CoreError::Bridge(format!(
                        "failed to create tensor 'reservoir_pre_decay': {e}"
                    ))
                })?;
        ...
        .map_err(|e| {
            CoreError::Bridge(format!(
                "failed to create tensor 'reservoir_hebbian_lr': {e}"
            ))
        })?;
        ...
        let tv =
            safetensors::tensor::TensorView::new(safetensors::Dtype::F32, vec![1], &decay_data)
                .map_err(|e| {
                    CoreError::Bridge(format!("failed to create tensor 'reservoir_decay': {e}"))
                })?;
    ...
    let tv = safetensors::tensor::TensorView::new(safetensors::Dtype::F32, vec![1], &kappa_data)
        .map_err(|e| {
            CoreError::Bridge(format!(
                "failed to create tensor 'cognition_kappa_gate': {e}"
            ))
        })?;
    tensors.insert("cognition_kappa_gate", tv);

    let tscale_data = c.t_scale.to_le_bytes().to_vec();
    let tv = safetensors::tensor::TensorView::new(safetensors::Dtype::F32, vec![1], &tscale_data)
        .map_err(|e| {
        CoreError::Bridge(format!("failed to create tensor 'cognition_t_scale': {e}"))
    })?;
    tensors.insert("cognition_t_scale", tv);

    let alpha_data = c.alpha_echo.to_le_bytes().to_vec();
    let tv = safetensors::tensor::TensorView::new(safetensors::Dtype::F32, vec![1], &alpha_data)
        .map_err(|e| {
            CoreError::Bridge(format!(
                "failed to create tensor 'cognition_alpha_echo': {e}"
            ))
        })?;
...
pub fn load_brain_state_extended(
    ...
    Array2<f32>,
    Array2<f32>,
    Option<crate::hippocampus::EchoReservoir>,
    crate::hippocampus::CognitionState,
)> {
    let file = std::fs::File::open(path)
        .map_err(|e| CoreError::Bridge(format!("failed to open {}: {}", path.display(), e)))?;
    let mmap = unsafe { memmap2::Mmap::map(&file) }
    ...
    let reservoir = if let Ok(assoc_tensor) = st.tensor("associations") {
        ...
        if assoc_shape.len() == 2 && assoc_shape[0] == 302 && assoc_shape[1] == 302 {
            ...
            let pre_decay = st
                .tensor("reservoir_pre_decay")
                .ok()
                .and_then(|t| bridge::tensor_bytes_to_f32_vec(t.data(), t.dtype()).ok())
                .and_then(|v| v.first().copied())
                .unwrap_or(0.99999);
            let hebbian_lr = st
                .tensor("reservoir_hebbian_lr")
                .ok()
                .and_then(|t| bridge::tensor_bytes_to_f32_vec(t.data(), t.dtype()).ok())
                .and_then(|v| v.first().copied())
                .unwrap_or(0.01);
            let decay = st
                .tensor("reservoir_decay")
                .ok()
                .and_then(|t| bridge::tensor_bytes_to_f32_vec(t.data(), t.dtype()).ok())
                .and_then(|v| v.first().copied())
                .unwrap_or(0.01);
    ...
    let cognition = crate::hippocampus::CognitionState {
        kappa_gate: st
            .tensor("cognition_kappa_gate")
            .ok()
            .and_then(|t| bridge::tensor_bytes_to_f32_vec(t.data(), t.dtype()).ok())
            .and_then(|v| v.first().copied())
            .unwrap_or(1.0),
        t_scale: st
            .tensor("cognition_t_scale")
            .ok()
            .and_then(|t| bridge::tensor_bytes_to_f32_vec(t.data(), t.dtype()).ok())
            .and_then(|v| v.first().copied())
            .unwrap_or(50.0),
        alpha_echo: st
            .tensor("cognition_alpha_echo")
            .ok()
            .and_then(|t| bridge::tensor_bytes_to_f32_vec(t.data(), t.dtype()).ok())
            .and_then(|v| v.first().copied())
            .unwrap_or(0.0),
...
mod tests {
    ...
    fn test_save_brain_state_roundtrip() {
        ...
        assert!(
            meta.len() > 364_000,
            "file too small for 302×302 F32 matrix"
        );
