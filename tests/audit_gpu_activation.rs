...
use utophiecorn_architecture::{
    geometry::token_to_coord, storage::load_brain_state, worm_brain::WormBrain,
};

#[cfg(feature = "cuda")]
use candle_core::Tensor;
#[cfg(feature = "cuda")]
use utophiecorn_architecture::{geometry::MANIFOLD_DIM, gpu::GpuEngine};

#[cfg(feature = "cuda")]
#[test]
fn audit_gpu_activation() {
    let (synapses, input_proj) =
        load_brain_state(std::path::Path::new("trained_worm_v1.safetensors"))
    ...
    println!(
        "  pre_synaptic: mean={:.8}, max={:.8}",
        cpu_pre.mean().unwrap(),
        cpu_pre.iter().cloned().fold(f32::NEG_INFINITY, f32::max)
    );
    println!(
        "  activation:   mean={:.8}, max={:.8}, non-zero={}/302, sum={:.8}",
        cpu_mean,
        cpu_max,
        cpu_nonzero,
        cpu_act.iter().sum::<f32>()
    );

    // GPU forward (tanh) — exakt wie gpu.rs PersistantGpuState
    let eng = GpuEngine::try_new()
        .expect("gpu engine")
        .expect("cuda device");
    let dev = eng.device();
    ...
    let proj_gpu = eng
        .host_to_tensor_2d_f32(&proj_flat, &[n, d], "proj")
        .unwrap();
    let syn_gpu = eng
        .host_to_tensor_2d_f32(&syn_flat, &[n, n], "syn")
        .unwrap();

    // proj @ coord → (302,)  via (302,16) @ (16,1)
    let coord_vec: Vec<f32> = coord.to_vec();
    let coord_2d = Tensor::new(coord_vec.as_slice(), dev)
        .unwrap()
        .reshape((d, 1))
        .unwrap();
    ...
    println!("\n[GPU tanh]");
    println!(
        "  activation:   mean={:.8}, max={:.8}, min={:.8}, non-zero={}/302, sum={:.8}",
    ...
    if gpu_max.abs() < 1e-10 && gpu_min.abs() < 1e-10 {
        ...
        println!(
            "  CPU/GPU-Divergenz: sparsemax (sum=1, 3/302 non-zero) vs tanh (saturates at -1)"
        );
    ...
    assert!(
        ...
        "GPU KOLLAPS: tanh liefert Nullvektor (max={})",
        gpu_max
    );
    ...
    println!("\n--- CPU/GPU VERGLEICH ---");
    println!(
        "  CPU sparsemax: mean={:.6}, max={:.6}, non-zero={}/302",
        cpu_mean, cpu_max, cpu_nonzero
    );
    println!(
        "  GPU tanh:      mean={:.6}, max={:.6}, min={:.6}",
